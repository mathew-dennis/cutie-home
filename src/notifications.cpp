			   ...
			   QVariantMap hints, int expire_timeout)
{
	...
	if (replaces_id != 0) {
		QMetaObject::invokeMethod(
			((QQuickView *)parent())->rootObject(),
			"delNotification", Q_ARG(QVariant, replaces_id));
	}
	QMetaObject::invokeMethod(
		((QQuickView *)parent())->rootObject(), "addNotification",
		Q_ARG(QVariant, summary), Q_ARG(QVariant, body),
		Q_ARG(QVariant, (replaces_id != 0) ? replaces_id : currentId));
	return (replaces_id != 0) ? replaces_id : currentId;
}

void Notifications::CloseNotification(uint id)
{
	QMetaObject::invokeMethod(((QQuickView *)parent())->rootObject(),
				  "delNotification", Q_ARG(QVariant, id));
}
