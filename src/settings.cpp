...
Settings::Settings(QObject *parent) : QObject(parent) {
    m_desktopFormat = QSettings::registerFormat("desktop", readDesktopFile, nullptr);
}
    ...
    for (int dirI = 0; dirI < dataDirList.count(); dirI++) {
        ...
        if (curAppDir->exists()) {
            ...
            for (int fileI = 0; fileI < entryFiles.count(); fileI++) {
                ...
                QSettings *curEntryFile = new QSettings(dataDirList.at(dirI) + "/autostart/" + curEntryFileName, m_desktopFormat);
